...
export default function EditorPage() {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (dragRef.current) {
        const drag = dragRef.current; // capture before async state update
        const dx = e.clientX - drag.startMouseX;
        const dy = e.clientY - drag.startMouseY;
        const dTx = (dx / w) * 100;
        const dTy = (dy / h) * 100;
        const idx = drag.photoIdx;
        const startTx = drag.startTx;
        const startTy = drag.startTy;
        setPhotos((prev) =>
          prev.map((p, i) =>
            i === idx
              ? { ...p, tx: startTx + dTx, ty: startTy + dTy }
              : p
          )
        );
      }
      ...
      if (resizeRef.current) {
        const resize = resizeRef.current; // capture before async state update
        const dx = e.clientX - resize.startMouseX;
        const dy = e.clientY - resize.startMouseY;
        const idx = resize.photoIdx;
        const startW = resize.startW;
        const startH = resize.startH;
        setPhotos((prev) =>
          prev.map((p, i) =>
            i === idx
              ? {
                  ...p,
                  w: Math.max(60, startW + dx),
                  h: Math.max(60, startH + dy),
                }
              : p
          )
        );
