...
// ─── Follow / parallax section ────────────────────────────────────────────────
function FollowSection({ locale }: { locale: string }) {
  const [offset, setOffset] = useState(0);
  const sectionRef = useRef<HTMLElement>(null);
  const reduced = useReducedMotion();

  useEffect(() => {
    if (reduced) return; // skip parallax entirely for motion-sensitive users
    const onScroll = () => {
  ...
  return (
    ...
    >
      {/* Parallax image — decorative, described by heading */}
      <div
        aria-hidden="true"
      ...
      >
        <img
          src="/images/sofie-follow-wide.jpg"
          alt=""
          role="presentation"
      ...
      <div aria-hidden="true" className="absolute inset-0" style={{ background: "linear-gradient(to right, rgba(26,10,13,0.72) 0%, rgba(26,10,13,0.18) 40%, rgba(26,10,13,0.18) 60%, rgba(26,10,13,0.72) 100%)" }} />
      <div aria-hidden="true" className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(26,10,13,0.25) 0%, transparent 30%, transparent 60%, rgba(26,10,13,0.55) 100%)" }} />

      {/* Content */}
      <div className="absolute inset-0 flex items-center">
        <div
          ref={textRef}
          className="px-10 md:px-20 max-w-lg"
          style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(24px)", transition: "opacity 1s ease, transform 1s ease" }}
        >
          <p className="eyebrow text-sand/60 mb-5 tracking-ultra-wide">
            {locale === "sv" ? "Följ resan" : "Follow the journey"}
          </p>
          <h2 className="font-serif text-4xl md:text-6xl font-light text-cream tracking-wide leading-tight mb-6">
            {locale === "sv" ? "Häng med\nbakoms kulisserna" : "Come behind\nthe scenes"}
          </h2>
          <p className="text-cream/70 text-sm font-sans leading-relaxed tracking-wide mb-10 max-w-xs">
            {locale === "sv"
          ...
          <div className="flex flex-col gap-3">
            <a
              ...
              className="inline-flex items-center gap-2.5 min-h-[44px] px-6 py-3 text-xs tracking-widest uppercase font-sans transition-all duration-300 hover:bg-white/20 focus-visible:outline-2 focus-visible:outline-sand focus-visible:outline-offset-2"
